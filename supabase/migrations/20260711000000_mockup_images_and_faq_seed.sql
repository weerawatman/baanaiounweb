...
SELECT v.question, v.answer, v.page_slug, v.sort_order
FROM (VALUES
  ('บริการจัดหาบ้านและคอนโด มีค่าใช้จ่ายแอบแฝงไหม?', 'บริการผู้ช่วยจัดหาทรัพย์ของเราฟรี 100% ไม่มีค่าใช้จ่ายแอบแฝงหรือบวกราคาเพิ่ม เราได้รับค่าการตลาดจากทางเจ้าของทรัพย์โดยตรง', 'find-property', 1),
  ('บ้านไออุ่นรับจัดหาอสังหาฯ ในทำเลไหนเป็นพิเศษบ้าง?', 'ครอบคลุมทำเล กรุงเทพมหานคร, สมุทรปราการ, ชลบุรี, ฉะเชิงเทรา, ระยอง และโซนนิคมอุตสาหกรรม EEC', 'find-property', 2),
  ('หลังจากส่งโจทย์ความต้องการแล้ว ใช้เวลาหาทรัพย์นานแค่ไหน?', 'โดยปกติเราจะส่งลิสต์ที่ผ่านการคัดกรองเบื้องต้นให้พิจารณาได้ภายใน 1-3 วันทำการ', 'find-property', 3),
  ('ทำไมการให้ทีมงานหาให้ ถึงดีกว่าการค้นหาดูประกาศด้วยตัวเอง?', 'ช่วยประหยัดเวลา และเราคัดกรองด้วยสายตานักลงทุน ตรวจสอบสภาพแวดล้อมและโครงสร้างเบื้องต้นก่อนพาคุณไปดูหน้างานจริง', 'find-property', 4)
) AS v(question, answer, page_slug, sort_order)
WHERE NOT EXISTS (SELECT 1 FROM public.faqs f WHERE f.page_slug = 'find-property');

INSERT INTO public.faqs (question, answer, page_slug, sort_order)
SELECT v.question, v.answer, v.page_slug, v.sort_order
FROM (VALUES
  ('ฝากขายบ้านกับบ้านไออุ่น ต่างจากการโพสต์ประกาศขายเองอย่างไร?', 'เราช่วยดูแลครบวงจรตั้งแต่การประเมินราคาตลาด ทำการตลาดเชิงรุก กระจายทรัพย์เข้าสู่เครือข่าย Co-Agent ไปจนถึงการดูแลเรื่องสัญญา', 'list-property', 1),
  ('บ้านไออุ่นรับฝากขายในทำเลไหนบ้าง?', 'ครอบคลุมทำเลศักยภาพใน กรุงเทพมหานคร, สมุทรปราการ, ชลบุรี, ฉะเชิงเทรา, ระยอง และโซน EEC', 'list-property', 2)
) AS v(question, answer, page_slug, sort_order)
WHERE NOT EXISTS (SELECT 1 FROM public.faqs f WHERE f.page_slug = 'list-property');
